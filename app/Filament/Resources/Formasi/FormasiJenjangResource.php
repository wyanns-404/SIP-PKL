<?php

namespace App\Filament\Resources\Formasi;

use App\Filament\Resources\Formasi\FormasiJenjangResource\Pages;
use App\Filament\Resources\Formasi\FormasiJenjangResource\RelationManagers;
use App\Models\Formasi\FormasiJenjang;
use Filament\Forms;
use Filament\Forms\Form;
use Filament\Resources\Resource;
use Filament\Tables;
use Filament\Tables\Table;
use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Eloquent\SoftDeletingScope;

class FormasiJenjangResource extends Resource
{
    protected static ?string $model = FormasiJenjang::class;

    protected static ?string $navigationIcon = 'heroicon-o-rectangle-stack';

    protected static ?string $label = 'Jenjang Pendidikan';

    protected static ?string $slug = 'formasi/jenjang-pendidikan';

    protected static ?string $breadcrumb = 'Jenjang Pendidikan';

    protected static ?string $pluralLabel = 'Jenjang Pendidikan';

    protected static ?string $navigationParentItem = 'Formasi PKL';

    protected static ?int $navigationSort = 4;

    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Forms\Components\TextInput::make('nama_jenjang')
                    ->label('Nama Jenjang')
                    ->required()
                    ->maxLength(255),
            ]);
    }

    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('nama_jenjang')
                    ->label('Nama Jenjang')
                    ->searchable(),
                Tables\Columns\TextColumn::make('created_at')
                    ->dateTime()
                    ->sortable()
                    ->toggleable(isToggledHiddenByDefault: true),
                Tables\Columns\TextColumn::make('updated_at')
                    ->dateTime()
                    ->sortable()
                    ->toggleable(isToggledHiddenByDefault: true),
            ])
            ->filters([
                //
            ])
            ->actions([
                Tables\Actions\EditAction::make(),
                Tables\Actions\DeleteAction::make(),
            ])
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\DeleteBulkAction::make(),
                ]),
            ]);
    }

    public static function getRelations(): array
    {
        return [
            //
        ];
    }

    public static function getPages(): array
    {
        return [
            'index' => Pages\ListFormasiJenjangs::route('/'),
        ];
    }
}
